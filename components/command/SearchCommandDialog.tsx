...
export function SearchCommandDialog() {
  ...
  const [rules, setRules] = useState<{ slug: string; title: string; description: string; tags: string[]; content: string }[]>([])
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    switch (e.key) {
      ...
      case 'ArrowUp':
        e.preventDefault();
        setSelectedIndex((i) => (i - 1 + filteredRules.length) % filteredRules.length);
        break;
      case 'Enter':
        e.preventDefault();
  ...
  return (
    <>
      ...
      <CommandDialog open={open} onOpenChange={handleOpenChange}>
        ...
        />
        {mode === 'rules' ? (
          <SearchRules searchQuery={searchQuery} selectedIndex={selectedIndex} setSelectedIndex={setSelectedIndex} />
        ) : (
